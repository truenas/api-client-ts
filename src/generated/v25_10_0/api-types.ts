...
/* ─────────────────────────────────────────────────────────────────────────────
 * virt.* — HAND-MAINTAINED, not produced by the generator.
 *
 * TrueNAS 25.10 shipped a `virt.*` namespace backed by Incus. It was removed
 * from middleware in b9c330ee94 (NAS-137115), and that commit deleted the model
 * files from every historical version directory as well as from master — so no
 * dump taken today describes it, and no regeneration can restore it. The API
 * itself is unchanged: 25.10 is released and frozen.
 *
 * Transcribed from `api/v25_10_5/virt_{global,instance,volume,device}.py` at tag
 * TS-25.10.5, where the four model files are blob-identical across v25_10_0
...
export interface VirtGlobalEntry {
  ...
  state: 'INITIALIZING' | 'INITIALIZED' | 'NO_POOL' | 'ERROR' | 'LOCKED' | null;
}

export interface VirtGlobalUpdate {
  pool?: string | null;
  bridge?: string | null;
  storage_pools?: string[] | null;
  v4_network?: string | null;
...
export interface VirtGlobalNetwork {
  type: 'BRIDGE';
  managed: boolean;
  ipv4_address: string;
  ipv4_nat: boolean;
...
export interface VirtInstanceAlias {
  type: 'INET' | 'INET6';
  address: string;
  netmask: number | null;
}

export interface VirtInstanceImage {
...
export interface VirtInstanceIdmapEntry {
  hostid: number;
  maprange: number;
  nsid: number;
}

export interface VirtInstanceUserNsIdmap {
  uid: VirtInstanceIdmapEntry | null;
  gid: VirtInstanceIdmapEntry | null;
}

export interface VirtInstanceEntry {
  id: string;
  name: string;
  type: 'CONTAINER' | 'VM';
  status:
  ...
  image: VirtInstanceImage;
  userns_idmap: VirtInstanceUserNsIdmap | null;
...
export type VirtInstanceQueryResultItem = Record<string, unknown>;

export interface VirtVolumeEntry {
  id: string;
  name: string;
  storage_pool: string;
  content_type: string;
...
export type VirtVolumeQueryResultItem = Record<string, unknown>;

export interface VirtDeviceBase {
  name?: string | null;
  description?: string | null;
  readonly?: boolean;
}

export interface VirtDeviceDisk extends VirtDeviceBase {
  dev_type: 'DISK';
  source?: string | null;
  destination?: string | null;
...
export interface VirtDeviceGpu extends VirtDeviceBase {
  dev_type: 'GPU';
...
export interface VirtDeviceProxy extends VirtDeviceBase {
  dev_type: 'PROXY';
...
export interface VirtDeviceTpm extends VirtDeviceBase {
  dev_type: 'TPM';
  path?: string | null;
  pathrm?: string | null;
}

export interface VirtDeviceUsb extends VirtDeviceBase {
  dev_type: 'USB';
  bus?: number | null;
  dev?: number | null;
  product_id?: string | null;
...
export interface VirtDeviceNic extends VirtDeviceBase {
  dev_type: 'NIC';
  network?: string | null;
...
export interface VirtDevicePci extends VirtDeviceBase {
  dev_type: 'PCI';
  address: string;
}

export interface VirtDeviceCdrom extends VirtDeviceBase {
  dev_type: 'CDROM';
  source: string;
  boot_priority?: number | null;
}

/** Discriminated on `dev_type`, as middleware declares it. */
export type VirtDeviceType =
  | VirtDeviceDisk
  | VirtDeviceGpu
  | VirtDeviceProxy
  | VirtDeviceTpm
...
export interface VirtInstanceCreate {
  name: string;
  image: string;
  source_type?: 'IMAGE';
...
export interface VirtInstanceStopOptions {
  timeout?: number;
  force?: boolean;
}

export interface VirtInstanceImageChoice {
  label: string;
  os: string;
  release: string;
  archs: string[];
...
export interface VirtVolumeCreate {
  name: string;
  content_type?: 'BLOCK';
  size?: number;
  storage_pool?: string | null;
...
export interface VirtVolumeUpdate {
  size?: number;
}

export interface VirtVolumeImportIso {
  name: string;
...
export interface VirtVolumeImportZvolItem {
  virt_volume_name: string;
  zvol_path: string;
}

export interface VirtVolumeImportZvol {
  to_import: VirtVolumeImportZvolItem[];
  clone?: boolean;
}

export interface VirtDeviceUsbChoice {
  vendor_id: string;
  product_id: string;
  bus: number;
  dev: number;
...
export interface VirtDeviceImportDiskImage {
  diskimg: string;
  zvol: string;
}
...
export interface VirtInstanceAddedEvent {
  id: string;
  fields: VirtInstanceEntry;
}

export interface VirtInstanceChangedEvent {
  id: string;
  fields: VirtInstanceEntry;
}

export interface VirtInstanceRemovedEvent {
  id: string;
}
