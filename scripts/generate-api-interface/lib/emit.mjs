...
const literal = (value) => (typeof value === 'string' ? quote(value) : JSON.stringify(value));
 ...
 * def root.
 */
function stripNestedTitles(node, isRoot = true) {
...
function usedByLine(usedBy, cap = 10) {
 ...
 */
export async function emitTypes(definitions, sharedNames = new Set()) {
  const objectDefs = {};
  const enumDefs = {};
  for (const [name, def] of Object.entries(definitions)) {
    ...
    (_kind === 'enum' ? enumDefs : objectDefs)[name] = stripNestedTitles(schema);
  }
...
function directoryEntry(method) {
...
function collectRefs(node, into) {
  if (Array.isArray(node)) {
    ...
    if (typeof node.$ref === 'string') into.add(refName(node.$ref));
    Object.values(node).forEach((v) => collectRefs(v, into));
  }
  return into;
}

function importLine(nodes, sharedNames = new Set()) {
  const names = [...collectRefs(nodes, new Set())].sort();
  const block = (list, from) => (list.length
    ? `import type {\n${list.map((n) => `  ${n},`).join('\n')}\n} from '${from}';\n`
    : '');
...
export function emitCallDirectory(methods, sharedNames) {
...
export function emitJobDirectory(methods, sharedNames) {
...
export function emitEventDirectory(events, sharedNames) {
 ...
 */
export function emitIndex({ sharedEnums = [], sharedTypes = [] } = {}) {
  const parts = [HEADER];
 ...
 */
export function emitRootIndex(versionDirs) {
