 ...
 * manifest says so in its own header — and middleware backports docstring edits
 * into released version directories routinely. Including it would fire the
 * drift check on changes that provably cannot affect a single emitted byte, and
 * a check that cries wolf is a check that gets re-blessed reflexively.
 ...
 * field named `description` is always an object.
 */
